# 修复计划：reply-to-comment-in-feed（二级回复稳定性）

## 目标
- 修复二级回复错位、正文污染与参数约束不一致的问题，提升通知页回复命中率与稳定性。

## 实施步骤
- [x] 修复回复正文污染：移除内部校验 marker 注入，发送内容保持与 `--content` 一致。
- [x] 通知页直回链路增加 `anchor_comment_id` 精准匹配，降低同文案串线风险。
- [x] `--target-comment-content` 改为可选增强参数（仍建议传入），与文档语义保持一致。
- [x] 回退到 feed 锚点回复时复用同一正文，不再修改内容。
- [x] 修复 feed 回退发送按钮定位：基于当前激活输入框所在局部容器（`engage-bar`）查找 `发送`，避免误选“回复/互动”控件。
- [x] 新增本地 `unittest` 回归测试，覆盖直回、回退、参数校验与正文一致性。
- [x] 成功判定改为“可验证投递”：必须命中 feed 详情/API/DOM 回查之一，否则直接报错（不再返回伪成功）。
- [x] 冒烟验证：在已登录测试账号执行真实二级回复（通知页直回未验证送达后自动回退 feed 锚点重试），`delivery_verified=true` 且 `delivery_check=comment_post_api`。
